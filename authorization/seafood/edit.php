...
<?php
  ...
  $id = $_GET['id'];
  $start = $_GET['start'];
  $result = mysqli_query($conn, "SELECT * FROM seafood WHERE id = $id");
...
<form action="update.php" method="post" enctype="multipart/form-data" id="commentForm">
  ...
  <?php $result_sub_categories = mysqli_query($conn, "SELECT * FROM sub_categories WHERE main_category = 'seafood'"); ?>
