...
global $conn;
    $total = mysqli_query($conn, "SELECT id FROM seafood");
    ...
    if(isset($_GET['type'])) {
        ...
        if($type !== 'myanmar' && $type !== 'chinese' && $type !== 'indian')
        {
            ...
            // get all records
            $all_records = get_all_records("seafood",$limit,$start);
        } else {
            $all_records = get_all_records_by_type("seafood",$type,$limit,$start);
        }
    } else {
        // get all records
        $all_records = get_all_records("seafood",$limit,$start);
...
?>
    <div class="with_background">
        <div class="with_background_child_element">
            ...
            <?php                 
                // get latest record
                $latest_record = mysqli_fetch_assoc(get_latest_record("seafood"));
            ...
            <?php
                // count total records
                if($type == 'all') {
                    $rs_result = total_count("seafood");  
                } else {
                    $rs_result = total_count_by_type("seafood",$type);
                }
